...
// 409 — { "error": "This slot has already passed" }   (elapsed slot from a stale list)
...
| Booking an elapsed slot today                             | Three layers: the slots query excludes `start_time <= now()`, the UI disables and dims any slot that elapses while on screen, and the booking transaction rejects stale submissions with `409 This slot has already passed`. |
